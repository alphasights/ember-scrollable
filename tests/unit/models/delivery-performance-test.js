...
test('#weekdayHoursSinceBeginningOfMonth calculates weekday hours including previous hours of the current day', function(assert) {
  ...
  assert.equal(this.model.weekdayHoursSinceBeginningOfMonth(), weekdayHours);
});

test('#totalWeekdayHoursInCurrentMonth calculates the number of weekday hours in the current month', function(assert) {
  ...
  assert.equal(this.model.totalWeekdayHoursInCurrentMonth(), weekdayHoursInMonth);
});
...
test('#numberOfWeekdayHoursUntil calculates weekday hours by excluding weekends', function(assert) {
  // February 1, 7, 8 were weekend days, leaving five weekdays before February 8.
  Timecop.freeze(moment('2015-02-08T00:00:00.000+00:00'));
  var totalHours = 5 * 24;

  assert.equal(this.model.numberOfWeekdayHoursUntil(8), totalHours);
});

test("#hoursSinceTodaysStart calculates the number of hours since the day's start", function(assert) {
  // February 20, 2015 12pm
  Timecop.freeze(moment('2015-02-20T12:00:00.000+00:00'));

  assert.equal(this.model.hoursSinceTodaysStart(), 12);
});

test("#hoursSinceTodaysStart handles rounding down the number of hours since the day's start", function(assert) {
...
test("#hoursSinceTodaysStart handles rounding up the number of hours since the day's start", function(assert) {
